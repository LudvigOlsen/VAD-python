...
class VoiceActivityDetector():
    ...
    def __init__(self, wave_input_filename=None, wave=None, rate=None):
        """
        :param wave_input_filename: Filename for reading wav file.
        :param wave: List/array with already read file (mono).
        :param rate: sample rate of wave (Only if wave is specified).
        """
        if (not wave_input_filename and not wave) or (wave_input_filename and wave):
            raise ValueError("Exactly one of wave_input_filename or wave must be specified.")

        if wave_input_filename:
            self._read_wav(wave_input_filename)._convert_to_mono()
        else:
            if not rate:
                raise ValueError("rate must be specified when passing wave")
            self.data = wave
            self.rate = rate
    ...
    def includes_speech(self, detected_windows, thresh=0):
        """
        1 if more speech than threshold, 0 if less speech than threshold.
        thresh is number of windows detected as speech.
        """
        speech_windows = np.matrix(detected_windows).sum(axis=0)[0, 1]
        return 1 if speech_windows > thresh else 0
